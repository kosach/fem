...
def dN_deta_2D(eta, tau, node_idx_2d, local_node_coords_2d):
    ...
    dval = 0.0
    if node_idx_2d < 4:
        dval = 0.25 * etai * (1 + tau * taui) * (2 * eta * etai + tau * taui - 1)
    else:
        if abs(etai) > 1e-6 and abs(taui) < 1e-6 :
            dval = 0.5 * etai * (1 - tau * tau)
        elif abs(taui) > 1e-6 and abs(etai) < 1e-6:
            dval = 0.5 * (-2 * eta) * (1 + tau * taui)
    return dval
...
def dN_dtau_2D(eta, tau, node_idx_2d, local_node_coords_2d):
    ...
    dval = 0.0
    if node_idx_2d < 4:
        dval = 0.25 * taui * (1 + eta * etai) * (eta * etai + 2 * tau * taui - 1)
    else:
        if abs(etai) > 1e-6 and abs(taui) < 1e-6 :
            dval = 0.5 * (1 + eta * etai) * (-2 * tau)
        elif abs(taui) > 1e-6 and abs(etai) < 1e-6:
            dval = 0.5 * (1 - eta * eta) * taui
    return dval
